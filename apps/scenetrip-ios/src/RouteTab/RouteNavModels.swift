...
// ## 서버가 아직 없다
//
// 계약의 `POST /navigation/next-leg` 는 지금 `501` 을 돌려준다. 어느 API 를 쓸지
// (T맵이냐 카카오냐)가 아직 정해지지 않았고, 그 위에 하루 호출 상한(MZ2AZ-205)도
// 미정이다. 그래서 이 타입들은 **모양만 정하고 값은 목으로 채운다** — 서버가 서면
// 이 타입을 채우는 쪽만 갈아 끼우면 화면은 그대로다.
