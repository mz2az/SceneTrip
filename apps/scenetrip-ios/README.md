...
마스코트는 진돗개 **「진도(JINDO)」**다(2026-08-28, 고양이 피노에서 교체 —
MZ2AZ-286). 정본은 일러스트 컷(`resources/Images.xcassets/jindo-*`)이고, 앱은
포즈별 그림을 갈아끼우며 소품(반짝이·말풍선)과 모션(숨쉬기·갸웃)만 코드로
얹는다 — `PinoMascot.swift` 머리말 참고. 코드 안 타입 이름(`Pino*`)은 교체
전 이름이 남은 것이다.
