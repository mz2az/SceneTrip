...
마스코트 **피노(PINO)** 는 새 그림이 아니라 지도 핀(`PinImage.numbered`)에 귀·얼굴·
꼬리를 붙인 것이다. GIF 가 아니라 SwiftUI `Path` 로 그린다 — 의존성도 용량도 늘지 않고
어느 배율에서도 선명하다.
...
경로여정 탭은 여러 세션에 걸쳐 만들어졌다. 아래는 마지막으로 커밋된 지점
(`958e694`, 2026-08-22 — 「경로여정 첫 화면을 확정 목업에 맞춘다」) 이후 무엇이 있었는지
날짜별로 남긴 것이다. **이 항목들은 아직 커밋되지 않았다** — `git status` 로 실제
diff 를 먼저 확인할 것.

`2026-08-22` 이전(동선 최적화 세 방법·확정 목업 반영까지)은 git 커밋 메시지가 정본이다
— `git log --oneline`.
