...
있을 때만). 안내가 켜지면 서울시청(마지막 자리)에서 경로선을 따라 걷고(도보 240 m/s ·
대중교통 두 배, 느리게 보려면 `-demoSpeed 12`), 핀 12 m 앞에서 멈춘다. 반경 100 m 에 **5초
머무르면** 스탬프가 찍히는데, 그 뒤에도 핀에 닿을 때까지 받은 길로 걷고 경로선도 그때까지
남는다. 코스 화면을 나가면 시청으로 되돌아간다.
