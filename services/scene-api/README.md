...
엔드포인트는 **15개**이고 전부 구현돼 있다. 경로는 명세의 `servers.url` 을 따라 `/v1`
아래에 있다.

> **경로여정 탭은 절반쯤 와 있다.** 명세(`scene-api-v1.yaml` v1.1.0)에 코스·작품찜·
> 마켓·길찾기 경로 18개가 있고, 그중 **코스 6개가 구현됐다**
> ([MZ2AZ-229](https://mz2az.atlassian.net/browse/MZ2AZ-229), 아래 표). 나머지는
> 아직 핸들러가 없어 `501` 이다 — 방문 체크(230), 작품 찜(231), 마켓(232),
> 길찾기(233). 구현 순서는 [계획 문서 §9](../../docs/project/plans/course-api.md).
>
