...
| `poi-sample.jsonl` | POI(편의시설) 표본 **23 행** — 갈래별 5 행 + 중복 한 쌍 + 버려질 행 하나. 김태환 아닌 정승길 수집(TMAP, 2026-08-26 판) |
| `poi.sql` | JSON Lines 를 `poi` 표로 옮기는 변환. `just seed-poi` 가 쓴다 |

```bash
just seed                          # 저장소의 성지후보 87 행
just seed <다른 CSV 경로>          # 같은 30 컬럼 형식이어야 한다
just seed-poi                      # 저장소의 POI 표본 23 행
...
**전량은 저장소에 없다.** TMAP 약관상 공개 배포가 안 되고 190 MB 다. 정승길이 준 파일
(`~/Downloads/압축 poi 2/`, 2026-08-26 판, 500,807 행)을 경로로 넘긴다. `just poi-filter`
가 원본 네 파일에서 허용목록 갈래만 남긴 파일을 만들고, 그 넷을 `just seed-poi` 에 **한
번에** 준다 — 파일을 넘나드는 중복(같은 가게가 음식·명소 양쪽에)을 한 번의 적재 안에서
접기 위해서다. 원본은 손대지 않는다.

**지우지 않는다.** 성지 시드와 달리 `TRUNCATE` 가 없다. 출처가 준 `source_id` 가
자연키라 `ON CONFLICT` 로 UPSERT 한다 — 있는 행은 갱신, 없는 행은 추가, 바뀐 것이
없으면 건드리지 않는다. 몇 번을 돌려도 안전하고, 좌표가 틀린 판을 넣었더라도 새 판을
다시 돌리면 고쳐진다.

**읽는 칸.** `id` `name` `lat` `lng` `kind` `biz_middle` `addr` `road` `tel` `region`
`city`. 세부 종류는 8/13 판이 `biz_lower` 에, 8/26 판이 `kind` 에 두므로 둘을 `COALESCE`
한다. `front_lat`·`front_lng`·`verified`·`keyword`·`biz_upper`·`area`·`near_spot` 은
읽지 않는다 — 이유는 `poi.sql` 머리와 poi.md.
