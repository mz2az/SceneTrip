...
-- 3일 코스에 4일차 항목을 넣어도 구조상 통과한다.
--
-- 그래도 트리거를 걸지 않는다. API 를 통해서는 그 상태가 만들어지지 않기 때문이다 —
-- 편집 요청은 항목을 일차 배열 안에 담고 day_no 는 배열 위치에서 서버가 계산한다.
-- 3일치를 보내면 4일차 항목을 표현할 방법 자체가 없다. 넘는 값이 들어오려면 psql 로
-- 직접 넣거나 API 밖의 경로가 생겨야 하고, 지금 그런 경로는 없다.
