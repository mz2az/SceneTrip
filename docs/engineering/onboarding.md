...
**앱(`apps/`)은 짓지 않는다.** 자바 언어 서버가 Swift 도 Kotlin/Android 도 읽지 않으므로
필요가 없고, 넣어 두면 Android SDK 나 Xcode 가 없는 기계에서 자바 코드 탐색까지 함께
막힌다. 그래서 **백엔드만 만지는 사람은 둘 다 없이도 `just ide` 를 쓸 수 있다.**
