...
**자바에 필요한 것만 짓는다** — `//services/...` 와 계약에서 생성된 자바 인터페이스. 앱은
짓지 않는다. 자바 언어 서버가 Swift 도 Kotlin/Android 도 읽지 않아 필요가 없고, 넣어 두면
Android SDK 나 Xcode 가 없는 기계에서 **자바 코드 탐색까지 함께 막힌다.**

그래서 **백엔드만 만지는 사람은 둘 다 없이도 `just ide` 를 쓸 수 있다.** 앱 쪽 IDE 는 따로
있다 — iOS 는 `just ios-xcode`, 안드로이드는 아직 없다.
