...
`course.status = 'ongoing'` 이 이 갈림길의 스위치다. 서버가 상태를 보고 길찾기 호출을
허용할지 정한다 — 클라이언트가 정하게 두면 과금이 클라이언트 손에 넘어간다.

**구현은 이번 범위 밖이다.** 선행 티켓
[MZ2AZ-204](https://mz2az.atlassian.net/browse/MZ2AZ-204)(T맵 종량제 연동 준비)와
[MZ2AZ-205](https://mz2az.atlassian.net/browse/MZ2AZ-205)(남용 방지 정책)가 끝나야
한다. 명세에 자리만 잡아 두고 핸들러는 `501` 을 낸다.

## 8. route-tab.md 와의 관계

[route-tab.md](./route-tab.md)(2026-08-09)는 **"촬영지 좌표가 고정이니 경로를 미리
계산해 DB 에 캐싱하자"** 를 전제로 OSRM 셀프호스팅과 `route_segment` 캐시 테이블을
검토한 문서다.

8-11 목업 확정이 그 전제를 뒤집었다. 계획 단계에서는 경로를 아예 그리지 않고, 여행
중에만 한 구간씩 T맵으로 부른다. 한 구간씩 부르는 값은 현재 위치에 따라 매번 다르므로
**캐싱할 대상이 아니다.** 그래서 이 계획에는 `route_segment` 도 OSRM 도 없다.

route-tab.md 는 지우지 않는다 — 네이버 약관 조사와 좌표 리매핑 분석은 그대로 유효하고,
해외 확장이나 "계획 단계에도 실제 경로를 보여 주자" 가 다시 논의되면 그 문서가 답을
들고 있다. 뒤집힌 사실은 그쪽 변경 이력에 덧붙였다.
