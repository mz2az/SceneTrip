...
**구현은 보류다 (2026-08-13).** 선행 티켓
[MZ2AZ-204](https://mz2az.atlassian.net/browse/MZ2AZ-204)(종량제 연동 준비)·
[MZ2AZ-205](https://mz2az.atlassian.net/browse/MZ2AZ-205)(남용 방지 정책)가 남은 데다,
**어느 API 를 쓸지 자체가 아직 안 정해졌다** — 카카오맵 쪽으로 기울어 있으나 미정이다
([route-tab.md](./route-tab.md) §13 이 그 논의의 정본).

그래서 명세에 자리만 잡아 두고 핸들러는 `501` 을 낸다. 요청·응답 모양도 고르는 API 에
따라 바뀔 수 있으므로 **이 경로에 의존하는 화면을 아직 만들지 않는다.** 지금 짐작으로
구현하면 API 가 정해질 때 두 번 일하게 된다.
