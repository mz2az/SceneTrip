...
**그런데 API 를 통해서는 그 상태가 만들어지지 않는다.** 편집 요청은 아이템을 일차 배열
**안에** 담고(§5-1), 일차 번호는 배열 위치에서 서버가 계산한다. 3일치를 보내면 4일차짜리
아이템을 표현할 방법 자체가 없다 — 클라이언트는 `day_no` 를 보내지 않는다.
...
| 바깥 API | 안 쓴다 | 부른다 — 어느 것인지는 [route-tab.md](./route-tab.md) §13 이 정본 |
| 과금 | 무료 | 유료 |
...
**구현은 이번 범위 밖이다.** 선행 티켓
[MZ2AZ-204](https://mz2az.atlassian.net/browse/MZ2AZ-204)(종량제 연동 준비)와
[MZ2AZ-205](https://mz2az.atlassian.net/browse/MZ2AZ-205)(남용 방지 정책)가 끝나야
한다. 명세에 자리만 잡아 두고 핸들러는 `501` 을 낸다.

## 8. route-tab.md 와의 관계

**둘은 다른 것을 다룬다.** [route-tab.md](./route-tab.md) 는 *경로를 어떻게 계산하는가*
— 어느 API 를 쓰고, 좌표를 어떻게 맞추고, 스파이크가 무엇을 답했는가. 이 문서는
*그 결과를 담을 표와 계약* 이다.

route-tab.md §11 이 이 계획의 전제를 준다 — **계획 단계에서는 직선거리만 쓰고 소요
시간을 감춘다.** 그래서 `CourseItem` 에 구간 소요 시간 필드가 없고, 이 계획에
`route_segment` 캐시 테이블도 OSRM 도 없다. 여행 중 한 구간씩 부르는 값은 현재 위치에
따라 매번 다르므로 캐싱할 대상이 아니다.

**바깥 API 가 무엇인지는 그쪽이 정본이다.** §13 에서 대중교통이 T맵에서 카카오로
갈렸듯 아직 움직이는 중이라, 이 문서에 옮겨 적으면 두 곳이 어긋난다.
[MZ2AZ-233](https://mz2az.atlassian.net/browse/MZ2AZ-233) 을 구현할 때 그 절을 다시
읽는다.
...
| 순서 | 티켓 | 산출물 | 상태 |
| --- | --- | --- | --- |
| 1 | 227 | 이 문서 + `V8`·`V9`·`V10` | **끝** — 실제 DB 에 적용, `just test-integration` 초록 |
| 1.5 | — | `UserStore`, `saved_place` 로 옮긴 장바구니 | **끝** — 실제 요청으로 확인 |
| 2 | 228 | `scene-api-v1.yaml` v1.1.0 + `just gen` | 진행 중 |
| 3 | 229 | `course/` 패키지 — 생성·목록·상세·완료(PUT)·삭제 | 대기 |
| 4 | 230 | 아이템 대조 로직, 소요 시간 계산, 방문 체크 | 대기 |
| 5 | 231 | `favorite/` 패키지 | 대기 |
| 6 | 232 | `market/` 패키지 | 대기 |
| 7 | 233 | 명세 자리 + `501` | 선행 티켓 대기 |
...
- [route-tab.md](./route-tab.md) — 경로 계산 조사 (§8 참고)
