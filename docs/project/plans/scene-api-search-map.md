...
| `GET /contents?q=` | 작품 제목·별칭 · 인물 이름 · 줄거리 · 장면 설명 |
| `GET /places?q=` | 장소명·별칭 · 등장 작품 제목 · 장소 설명 · 장면 설명 |

목록 검색의 설명 부분은 `search_term` 이 아니라 원본 테이블의 텍스트 컬럼을
직접 훑는다. 한국어는 조사가 붙어("감자탕**을**") PostgreSQL 기본 전문검색이
...
### 데이터 편차 — 지금은 장면 설명만 실하다

| 텍스트 | 상태 |
| --- | --- |
| 장면 설명 (`place_content_i18n.relation_description`) | **164/164 채움**, 평균 46 자 |
| 작품 줄거리 (`content_i18n.description`) | MVP 4 작품 중 1 개만 (작품마스터 전체로도 476 중 12) |
| 장소 설명 (`place_i18n.description`) | 수집 컬럼 자체가 없음 |

**명세는 셋 다 검색 대상으로 적는다.** 데이터가 채워지는 대로 계약을 바꾸지 않고
동작하게 하기 위해서다. 지금 실제로 걸리는 것은 장면 설명뿐이다.

장면 설명은 검색어로서 값어치가 높다 — `감자탕`, `데이트 장소` 처럼 사용자가 실제로
칠 법한 구체적 낱말이 들어 있다.
