...
| `GET /places?q=` | 장소명·별칭 · **장소 설명**(`place_i18n.description`) · **그리고 위 줄 전부** (등장 작품을 통해) |

**두 목록은 같은 텍스트 뭉치를 본다** — 결과를 작품으로 내느냐 장소로 내느냐만
다르다. 처음에는 장소 쪽을 "등장 작품 제목" 까지로만 잡았는데, 그러면 `공유` 를 쳤을
때 작품 탭에는 도깨비가 뜨는데 **장소 탭만 빈다.** 사용자는 탭을 먼저 고르지 않으므로
한쪽만 넓게 걸리면 나머지 탭이 고장난 것처럼 보인다. 장소는 `place_content` 로 작품에
이어져 있으니 작품 쪽에 걸린 것은 그 작품의 촬영지로 옮긴다.
...
그래서 `GET /places?q=` 는 **작품 쪽 텍스트 전부**(제목·별칭·설명·출연진 이름)에도
걸린다. 등장 작품을 거쳐 장소로 옮기는 것이다.

이것을 처음에는 "등장 작품 제목" 까지로만 잡았다. 그러면 이렇게 된다.
...
두 번째가 사용자에게는 고장이다. 목업의 검색창 안내가 `작품, 배우, 장면 검색` 이라
배우 이름을 치는 것이 정상 사용인데, 그때만 탭 하나가 빈다. **한쪽 탭에 걸리는
것은 다른 탭에도 걸려야 한다** — 결과의 종류만 다를 뿐 질문은 하나다.
...
| 6 | ~~API 사용 문서~~ | **해결됨.** `docs/api/` 에 `scene-api-guide.md`·`errors.md`·`auth.md` 를 썼다. 처음에 `errors.md` 만 후속으로 잡고 사용 가이드는 아예 빠뜨렸는데, 프론트에 넘길 내용(v1 에 없는 필드, 알려진 한계)을 PR 본문에만 적어 둔 상태였다 — PR 본문은 몇 주 뒤 아무도 읽지 않는다. 버전 정책은 `contracts/README.md` 와 중복이라 쓰지 않고 링크만 걸었다 |
