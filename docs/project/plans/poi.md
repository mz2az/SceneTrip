...
근거는 §10-1 의 실측이다. 요약하면 — **네 갈래가 갈리는 쌍은 0건**이라 합쳐도 분류를
잃지 않고, 전화번호가 갈리는 쌍 3건 중 둘은 오타다.
...
**`category_group`(네 갈래)이 갈리는 쌍은 0건이다.** 합쳐도 `food`/`sight`/`stay`/`transit`
구분을 잃지 않는다 — 이것이 합치기로 한 가장 큰 근거다.
