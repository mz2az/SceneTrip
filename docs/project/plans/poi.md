...
### 5-4. 출처 교체 — 공공데이터 판 (2026-09-09)

TMAP 을 쓰지 않는다(2026-09-05 팀 결정, [ADR 0014](../../architecture/adr/0014-poi-source-is-public-data.md)).
승길이 만든 `SceneTrip_POI_20260907/out` 여섯 파일이 정본이다. 열두 키 모양이 TMAP 판과 같아
`poi.sql` 이 읽는 칸은 그대로이고, 규칙 셋이 늘었다.

| 파일 | 행 | 출처 | id |
| --- | --- | --- | --- |
| `poi_food` | 840,255 | 소상공인 상가정보 「음식」 전체 | `MA…` |
| `poi_stay` | 80,483 | 상가정보 「숙박」 전체 | `MA…` |
| `poi_sight` | 33,285 | 관광공사 TourAPI (관광지·문화시설·축제·레포츠·쇼핑) | `tour-…` |
| `poi_tour_food` · `poi_tour_stay` | 13,495 · 2,939 | 관광공사 음식점·숙박 | `tour-…` |
| `poi_transit` | 2,495 | 공항·철도역·도시철도·터미널 정류소·환승센터 | 그 외 |

**규칙 1 — 좌표 범위.** 33~39N·124~132E 밖이면 버린다. 69 행. 위·경도가 바뀐 것들이다.

**규칙 2 — 관광공사 음식·숙박은 상가정보에 없는 것만.** 같은 가게가 두 출처에 있으면 좌표가
몇 m 어긋나 §5-2 의 1 m 규칙에 안 걸린다 — 관광공사 음식점 13,495 중 12,921 이 상가정보 행과
100 m 안, 10,291 은 이름도 겹친다(실측). 같은 갈래의 상가정보 행이 100 m 안에 있고 이름이 서로
포함(공백 무시)이면 관광공사 쪽을 넣지 않는다. 뺀 것 음식 10,100 · 숙박 1,572, 남은 것 음식
3,394 · 숙박 1,363. `origin` 은 id 생김새(`tour-`·`MA`)로 안다. 관광(sight)·교통은 상가정보에
없는 갈래라 이 규칙을 안 탄다.
...
나머지가 전부 같다. §5-2 의 1 m 규칙이 접는다. 전화번호 칸이 없어 음식·숙박은 `tel` 이 전부
빈 값이다 — 카드의 전화는 네이버가 채울 때만 보인다. 층이 다른 같은 상호는 접힌다 — 문제가
보이면 층을 키에 넣는다.

**달라지는 것.** 종교 갈래(사찰·교회 12,027)가 없어진다(관광공사 관광지에 유명 사찰은 있다).
주소 형식이 섞인다 — 상가정보는 동까지, 관광공사는 도로명 전체. 계약 `PoiSummary.address`
설명을 그에 맞췄다. `just poi-filter` 는 필요 없어졌다(전부 허용목록 안) — 남겨 둔다.

적재 결과(2026-09-09, kind) — 입력 973,000 행 → 좌표 밖 69 · 중복 접힘 12,353 · 관광공사 겹침
11,672 → **948,858 행** 적재, TMAP 500,516 행 삭제, **2분 57초**. food 833,220 · stay 79,979 ·
sight 33,220 · transit 2,439. `just poi-smoke` 통과(강남 2 km `total` 3,395, 상세 31 ms).

**아직 없는 것.** 외국어 이름 — 관광공사 `src` 에 영문·일문이 붙어 오지만 지금은 안 읽는다
(2026-09-09 결정). 관광공사 상세(영업시간·사진)는 하루 250 건씩 쌓이는 중이다.
...
| 2026-09-09 | 출처 교체(§5-4). TMAP → 공공데이터(상가정보·관광공사·교통) 948,858 행. 좌표 범위·관광공사↔상가정보 겹침 제외·`--prune` 규칙 추가. 외국어 이름은 보류 |
