...
> **보정 안내 (ADR 0005).** 이 ADR 의 **결정은 그대로 유효하다.** 다만 아래 "결과" 에 적힌
> "`just ci` 가 파이프라인을 로컬에서 그대로 재현한다" 는 전제가 한때 깨졌다 — 통합
> 테스트 레인에 실제 DB 가 필요해지면서, CI 러너에 그 의존성을 공급하는 방법이 정해져
> 있지 않았기 때문이다. 무엇을 공급하고 누가 공급하는지는
> [ADR 0005](./0005-integration-test-dependencies-in-ci.md) 가 정한다. 그 결정이 이행되면
> 이 ADR 의 전제와 검증 기준("어긋나는 경우가 레시피가 틀렸을 때뿐")은 다시 성립한다.
...
| Nx 또는 Turborepo | JS/TS 모노레포에는 강하지만 같은 트리의 JVM(Java)·Swift·Kotlin·Python 에는 약하다. 우리는 처음부터 넷 다 예상한다. |
