...
못한다. **의도한 상태다** — 마켓 로직 자체는 구현돼 있고 통합 테스트가 실제 DB 로
확인한다. 로그인이 `app_user.registered_at` 을 채우는 순간 벽이 열린다.

**로컬 kind 만 예외다.** 시뮬레이터에서 길찾기를 실제 서버로 보려고 그 클러스터의
ConfigMap 이 `SCENETRIP_AUTH_REQUIRE_REGISTRATION=false` 로 벽을 치워 두었다
(MZ2AZ-302). 그래서 로컬에서는 이 넷이 401 없이 통과한다. 앱은 이 사실을 모르고,
401 처리를 그대로 가지고 있다 — 원격에서는 벽이 그대로다.
