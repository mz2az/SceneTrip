...
### 실제로 띄워서 확인한 것 (2026-09-08)

scene-api(kind 클러스터)와 DeepSeek 을 붙이고 `POST /guide/chat` 으로 권호님 §2 의
시나리오를 그대로 돌렸다.

```
[1] 「도깨비로 1박 2일 짜 줘」        에이전트 1일차 5곳
[2] 앱에서 손으로 「서울 용답역 육교」 삭제   앱 1일차 4곳 (서버로 안 나감)
[3] 「1일차에서 서울 삼청동 돌담길 빼 줘」 + context.plan(4곳)
    → revise_plan 호출, 결과 3곳
      지운 「서울 용답역 육교」 되살아났나 : 아니오 ✔
      빼 달라고 한 곳 빠졌나            : 예   ✔
```

`POST /plan` 응답도 실제 정수 `placeId`(22·78·42·60·38)와 좌표를 싣고, `minutesToNext`
는 어디에도 없으며 한글 키도 남지 않았다.

### 남은 한 가지 — 막지 못한 것

`context.plan` 을 **일부러 빼고** 「2일차에서 하나 빼 줘」를 물으면, 모델이 대화 이력에
남은 옛 일정을 문장으로 읊는다. 「지금 상태」에 「짜 둔 일정: 없다」를 넣고, 시스템
프롬프트에 규칙을 더하고, 별도 시스템 메시지로 세워도 마찬가지였다(실측 3회).

**상태는 상하지 않는다** — 도구가 「짜 둔 일정이 없다」로 거절하므로 `effects` 가
나가지 않고 일정도 바뀌지 않는다. 남는 것은 답변 문구뿐이다. 그리고 실제 운영에서는
앱이 일정이 있는 한 늘 `context.plan` 을 보내므로(handoff §5) 이 자리에 들어오지 않는다.

프롬프트를 더 조이는 것으로는 안 되는 것으로 보이고, 대화 이력 자체를 손대야 하는
문제라 별도 티켓으로 남긴다.
...
- 일정이 없을 때 모델이 대화 이력에서 옛 일정을 읊는 것 — 위 §2 「막지 못한 것」. 별도 티켓.
