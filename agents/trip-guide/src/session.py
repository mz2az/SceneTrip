    ...
    def context_block(self) -> str:
        ...
        **좌표를 넣지 않는다.** 숫자를 보여 주면 모델이 그것으로 거리를 계산하려
        ...
        if self.plan is not None:
            ...
            # **없다고 명시한다.** 예전에는 아무 말도 하지 않아서, 상태의 정본이
            # 이 블록이라는 것을 모델이 알 길이 없었다.
            #
            # **다만 이것으로 다 막히지는 않는다.** `context.plan` 을 빼고 물었더니
            # 모델이 대화 이력에 남은 옛 일정을 그대로 읊었다 — 이 줄을 넣고도,
            # 시스템 프롬프트에 규칙을 더하고도, 별도 시스템 메시지로 세워 보고도
            # 마찬가지였다 (2026-09-08 실측 3회). **상태는 상하지 않는다** —
            # 도구가 「짜 둔 일정이 없다」 로 거절하므로 effects 가 나가지 않는다.
            # 남는 것은 답변 문구뿐이고, 실제 운영에서는 앱이 일정이 있는 한 늘
            # `context.plan` 을 보내므로 이 자리에 들어오지 않는다.
            lines.append(
                "- 짜 둔 일정: **없다.** 앞 대화에서 일정을 말했더라도 지금은 없는 "
                "것이다. 기억에서 되짚어 읊지 말고, 고쳐 달라는 요청에는 다시 짜야 "
                "한다고 안내하라."
            )
        return "\n".join(lines)
